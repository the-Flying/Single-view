...
# 第五张
img2 = cv2.imread("i5.png", flags=1)
...
# 第六张
img3 = cv2.imread("i6.png", flags=1)
height3, width3 = img3.shape[:2]  # 512 512
print(height3, width3)
# src = cv2.resize(img, (300, 300))
gray3 = cv2.cvtColor(img3, cv2.COLOR_BGR2GRAY)
_, binary3 = cv2.threshold(gray3, 200, 255, cv2.THRESH_BINARY_INV)

# 寻找二值化图中的轮廓，检索所有轮廓，输出轮廓的每个像素点
# contours, hierarchy = cv2.findContours(binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)  # OpenCV4~
binary3, contours3, hierarchy3 = cv2.findContours(binary3, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # OpenCV3

fullCnts3 = np.zeros(img3.shape[:2], np.uint8)  # 绘制轮廓函数会修改原始图像
fullCnts3 = cv2.drawContours(fullCnts3, contours3, -1, (255, 255, 255), thickness=0)  # 绘制全部轮廓

# 按轮廓的面积排序，绘制面积最大的轮廓
cnts3 = sorted(contours3, key=cv2.contourArea, reverse=True)  # 所有轮廓按面积排序
cnt3 = cnts3[0]  # 第 0 个轮廓，面积最大的轮廓，(1445, 1, 2)
maxCnt3 = np.zeros(img3.shape[:2], np.uint8)  # 初始化最大轮廓图像
cv2.drawContours(maxCnt3, cnts3[0], -1, (255, 255, 255), thickness=3)  # 仅绘制最大轮廓 cnt
print("len(contours3) =", len(contours3))  # contours 所有轮廓的数量
print("area of max contour3: ", cv2.contourArea(cnt3))  # 轮廓面积
print("perimeter of max contour3: {:.1f}".format(cv2.arcLength(cnt3, True)))  # 轮廓周长

# 主成分分析方法提取目标的方向
markedCnt3 = maxCnt3.copy()
ptsXY3 = np.squeeze(cnt3).astype(np.float64)  # 删除维度为1的数组维度，(1445, 1, 2)->(1445, 2)
mean3, eigenvectors3, eigenvalues3 = cv2.PCACompute2(ptsXY3, np.array([]))  # (1, 2) (2, 2) (2, 1)
print("mean3:{}, eigenvalues3:{}".format(mean3.round(1), eigenvalues3[:, 0].round(2)))

# 显示图像
plt.figure(figsize=(9, 6))
plt.subplot(331), plt.axis('off'), plt.title("Origin4")
plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
plt.subplot(332), plt.axis('off'), plt.title("Binary4")
plt.imshow(binary, 'gray')
plt.subplot(333), plt.axis('off'), plt.title("Contour4")
plt.imshow(fullCnts, 'gray')

plt.subplot(334), plt.axis('off'), plt.title("Origin5")
plt.imshow(cv2.cvtColor(img2, cv2.COLOR_BGR2RGB))
plt.subplot(335), plt.axis('off'), plt.title("Binary5")
plt.imshow(binary2, 'gray')
plt.subplot(336), plt.axis('off'), plt.title("Contour5")
plt.imshow(fullCnts2, 'gray')

plt.subplot(337), plt.axis('off'), plt.title("Origin6")
plt.imshow(cv2.cvtColor(img3, cv2.COLOR_BGR2RGB))
plt.subplot(338), plt.axis('off'), plt.title("Binary6")
plt.imshow(binary3, 'gray')
plt.subplot(339), plt.axis('off'), plt.title("Contour6")
plt.imshow(fullCnts3, 'gray')
